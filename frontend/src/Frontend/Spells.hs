...
blessingsMap = Map.fromList . fmap blessingMeta . DMap.toList
  where
    blessingMeta :: DSum Blessings Identity -> (EffectName, EffectMeta)
    blessingMeta e = case e of
      ...
      (Chastise :=> _) -> toEffectTuple "Chastise"
      ...
      (RefugeOFaith :=> _) -> toEffectTuple "Refuge o' Faith"
      ...
      (LayOnHands :=> _) -> toEffectTuple "Lay on Hands"
      ...
      (HolyRoller :=> _) -> toEffectTuple "Holy Roller"
      ...
      (Protection :=> _) -> toEffectTuple "Protection"
      ...
      (Confession :=> _) -> toEffectTuple "Confession"
      ...
      (MagicResistant :=> _) -> toEffectTuple "ResistMagic"
...
edgesMap :: EdgesMap -> Map EffectName EffectMeta
edgesMap = Map.fromList . fmap edgeMeta . DMap.toList
  where
    edgeMeta :: DSum Edges Identity -> (EffectName,EffectMeta)
    edgeMeta e = case e of
      (ArcaneBlessed :=> _) -> toEffectTuple "Arcane (Blessed)"
        "Gives the ability to cast blessed based spells."
        []
      (Brave :=> _)         -> toEffectTuple "Brave"
        "+2 to Guts checks"
      ...
      (LevelHeaded :=> _)  -> toEffectTuple "Level Headed"
      ...
      (NervesOfSteel :=> _) -> toEffectTuple "Nerves of Steel"
      ...
      (TheStare :=> _)     -> toEffectTuple "The Stare"
...
hinderancesMap :: HinderancesMap -> Map EffectName EffectMeta
hinderancesMap = Map.fromList . fmap hinderanceMeta . DMap.toList
  where
    hinderanceMeta :: DSum Hinderances Identity -> (EffectName, EffectMeta)
    hinderanceMeta h = case h of
      (Heroic :=> _) -> toEffectTuple "Heroic"
      ...
      (OathChurch :=> _) -> toEffectTuple "Oath (Church)"
        "Uphold the Catholic Church"
        []
      (Ferner :=> _) -> toEffectTuple "Ferner"
        "She's not from around here"
        []
      (Poverty :=> _) -> toEffectTuple "Poverty"
        "Donates money whereever she can. Lives simply."
        []

knacksMap :: KnacksMap -> Map EffectName EffectMeta
knacksMap = Map.fromList . fmap knackMeta . DMap.toList
  where
    knackMeta :: DSum Knacks Identity -> (EffectName, EffectMeta)
