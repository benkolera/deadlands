...
blessingsMap :: Set Blessings -> Map EffectName EffectMeta
blessingsMap = Map.fromList . fmap blessingMeta . toList
  where
    blessingMeta e = case e of
      ArmorOfRighteousness _ -> toEffectTuple "Armor o' Righteousness"
      ...
      Chastise -> toEffectTuple "Chastise"
      ...
      RefugeOFaith -> toEffectTuple "Refuge o' Faith"
      ...
      LayOnHands -> toEffectTuple "Lay on Hands"
      ...
      HolyRoller -> toEffectTuple "Holy Roller"
      ...
      Protection -> toEffectTuple "Protection"
      ...
      Confession -> toEffectTuple "Confession"
      ...
      MagicResistant -> toEffectTuple "ResistMagic"
...
edgesMap :: Set Edges -> Map EffectName EffectMeta
edgesMap = Map.fromList . fmap edgeMeta . toList
  where
    edgeMeta e = case e of
      ArcaneBlessed -> toEffectTuple "Arcane (Blessed)"
        "Gives the ability to cast blessed based spells."
        []
      Brave         -> toEffectTuple "Brave"
        "+2 to Guts checks"
      ...
      LevelHeaded   -> toEffectTuple "Level Headed"
      ...
      NervesOfSteel -> toEffectTuple "Nerves of Steel"
      ...
      TheStare      -> toEffectTuple "The Stare"
...
hinderancesMap :: Set Hinderances -> Map EffectName EffectMeta
hinderancesMap = Map.fromList . fmap hinderanceMeta . toList
  where
    hinderanceMeta h = case h of
      Heroic -> toEffectTuple "Heroic"
      ...
      OathChurch -> toEffectTuple "Oath (Church)"
        "Uphold the Catholic Church"
        []
      Ferner -> toEffectTuple "Ferner"
        "She's not from around here"
        []
      Poverty -> toEffectTuple "Poverty"
        "Donates money whereever she can. Lives simply."
        []

knacksMap :: Set Knacks -> Map EffectName EffectMeta
knacksMap = Map.fromList . fmap knackMeta . toList
  where
    knackMeta h = case h of
      BornOnChristmas -> toEffectTuple "Born On Christmas"
        ""
        []
...
blessings
  ...
  => Dynamic t (Set Blessings)
  -> m (Event t (Endo EffectMap))
blessings = effectsSection "Blessings" . fmap blessingsMap
